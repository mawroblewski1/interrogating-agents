...
include this targeting stage; we return to it in the Conclusion.
...
\begin{figure}[h]
  ...
  \caption{Directional accuracy (\%) by topic and condition. The dashed line at
    50\% marks direction-agnostic drift. Treatment exceeds control on 3 of 4
    topics; Arts is the exception (60\% = 60\%).}
  \label{fig:fig3}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Stance-change magnitude (boxplots with individual points) by topic
    and condition. The pooled panel (rightmost) shows overlapping distributions
    with similar means. Magnitude is a secondary metric due to judge scale drift
    on complex propositions; directional accuracy (Figure~\ref{fig:fig3}) is more
    reliable.}
  \label{fig:fig1}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Mean final stance (turn 6) by condition and initial suspect stance
    ($\pm$1 SEM); line colour encodes initial stance ($+2$ = blue, $-2$ = red).
    Treatment generally moves suspects toward the Interrogator's position, but
    appears less effective than control for initial stance $-2$ on Housing and
    Arts. Turn-0 baseline scores are unavailable in the current results.
    See Figure~\ref{fig:fig3} for directional accuracy.}
  \label{fig:fig2}
\end{figure}
...
Proposed and developed the overall three-role multi-agent architecture (Suspect, Interrogator,
Judge), the quad-based experimental design, the real-data generalization test
(the Davis Measure~V condition) and social network intervention application. Composed proposal document. Led the team and organized the collaborative
working meetings, and maintained the project's GitHub infrastructure. Finalized
the slides for the in-class presentation. Contributed to writing and revising the paper, and refining the figures and plots.
...
The majority of the project's progress was made collaboratively during the team's working
