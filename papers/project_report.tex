...
\begin{figure}[h]
  ...
  \caption{Mean final stance (turn 6) by condition and initial suspect stance
    ($\pm$1 SEM). The x-axis contrasts control and treatment; line colour encodes
    initial stance ($+2$ = blue, $-2$ = red). Only the final-turn score is
    available in the current results (per-turn logging is a planned extension);
    the baseline reference lines originally intended to show turn-0 scores are
    therefore omitted. See Figure~\ref{fig:fig3} for directional accuracy.}
  \label{fig:fig2}
\end{figure}
