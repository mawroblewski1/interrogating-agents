#!/usr/bin/env bash
#
# run_pipeline.sh — driver for the Parler persona pipeline (Kubuntu / bash)
#
# TWO MODES:
#
# 1) Numbered-stage mode (default) — runs stages 1-5 of the pipeline (see README.md),
#    with a --from/--to range so you can (re)run just part of it.
#
#      ./run_pipeline.sh --input /path/to/parler_folder [options]
#
#    Stage numbers:
...
#
#    Examples:
#      ./run_pipeline.sh --input "/mnt/d/Parler" --limit 40000        # smoke test, stages 1-4
#      ./run_pipeline.sh --input "/mnt/d/Parler"                      # full run, stages 1-4
#      ./run_pipeline.sh --input "/mnt/d/Parler" --from 5 --to 5      # just the final histories pass
#      ./run_pipeline.sh --input "/mnt/d/Parler" --stage 3            # rerun just step 3
#
...
#
#      ./run_pipeline.sh --scrape-lexicon --urls sources.txt [options]
#
...
#      ./run_pipeline.sh --scrape-lexicon --urls sources.txt --classify-backend local
#      ./run_pipeline.sh --scrape-lexicon --urls sources.txt --scrape-max-depth 1 --scrape-max-terms 500
#
set -euo pipefail
